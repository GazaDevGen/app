...
export default function Index() {
  ...
  const updateOrderStatusFunc = async () => {
    const { status } = await axios.put(`${BASE_URL}/api/orders/status/${orderId}`, { status: changeStatus })
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      <View>
        ...
        {/* Table Rows */}
        {filteredOrders.length >= 1 ? filteredOrders?.map((order, index) =>
          <View key={index} style={styles.tableRow}>
            ...
            <Pressable
              ...
              isHovered && clickedItem === order._id && { backgroundColor: "#333" }, // Changes background on press
              ]}
            >
              <Text
                style={styles.tableCell}
              >
                <Link href={{ pathname: "/orderDetail/[id]", params: { id: order._id } }}>{order.customerName}</Link></Text>
            ...
            >
              {order.status === 'pending' ? 'لم يتم القص ' : order.status === 'cut' ? 'تم قصها' :
                order.status === 'workingIn' ? 'تم تركيبها' : order.status === 'approved' ? 'جاهزة' : ''}
            </Text>
            <Text
              style={[styles.tableCell, { backgroundColor: isHovered && clickedItem === order._id ? 'red' : 'transparent' }]}
